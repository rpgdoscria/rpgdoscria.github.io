    ...
    if (charsRes.status === "fulfilled" && charsRes.value.characters) {
      ...
      if (chars.length === 0) {
        charsHtml = `
          ...
          <a href="criar-personagem.html" class="btn btn-primary btn-sm">+ Criar primeiro personagem</a>
        `;
      } else {
        charsHtml = chars.map(c => `
          <a href="criar-personagem.html?id=${c.id}" class="perfil-char-mini">
            ${avatarHtml(c)}
    ...
    if (pagesRes.status === "fulfilled") {
      ...
      if (myRecent.length === 0) {
        wikiHtml = `<p class="muted text-sm">Você ainda não editou páginas da wiki.</p>
                    <a href="wiki/index.html" class="btn btn-ghost btn-sm">Explorar wiki →</a>`;
      } else {
        wikiHtml = myRecent.map(r => {
          ...
          return `<a href="wiki/pagina.html?slug=${encodeURIComponent(slug)}" class="perfil-list-item">
      ...
      if (roomsRes.status === "fulfilled" && roomsRes.value.rooms) {
        ...
        if (rooms.length === 0) {
          roomsHtml = `<p class="muted text-sm">Você ainda não criou salas.</p>
                       <a href="criar-sala.html" class="btn btn-primary btn-sm">+ Criar primeira sala</a>`;
        } else {
          roomsHtml = rooms.slice(0, 10).map(r => `
            <div class="perfil-room-row ${r.isActive ? "" : "inactive"}">
              ...
              ${r.isActive ? `<a class="btn btn-sm btn-primary" href="sala.html?code=${encodeURIComponent(r.code)}">Reabrir</a>` : `<span class="tag tag-off">encerrada</span>`}
            </div>
          `).join("");
          if (rooms.length > 10) {
            roomsHtml += `<p class="text-xs muted mt-2">+${rooms.length - 10} salas anteriores — <a href="criar-sala.html">ver todas</a></p>`;
          }
        }
      } else {
    ...
    grid.innerHTML = `
      ...
      <div class="perfil-card">
        ...
        <div class="flex flex-wrap gap-2">
          <a href="wiki/index.html" class="btn btn-ghost btn-sm">📚 Wiki</a>
          <a href="entrar-sala.html" class="btn btn-ghost btn-sm">🔗 Entrar em sala</a>
          ${isMaster ? `<a href="criar-sala.html" class="btn btn-ghost btn-sm">🎪 Criar sala</a>` : ""}
          ${sess.user.role === "admin" ? `<a href="admin.html" class="btn btn-ghost btn-sm">⚙️ Admin</a>` : ""}
        </div>
      </div>
    `;
  }
